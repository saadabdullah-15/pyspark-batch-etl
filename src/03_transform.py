...
def with_payment_labels(trips: DataFrame) -> DataFrame:
...
def main() -> None:
    require_files(CLEAN_DIR / "yellow_taxi_trips", CLEAN_DIR / "taxi_zones")
    ...
    try:
        ...
        for table_name, dataframe in outputs.items():
            writer = dataframe.write.mode("overwrite").option("maxRecordsPerFile", 250000)
